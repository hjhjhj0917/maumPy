...
# =========================================================
# 1. Train / Test Split (최우선 실행 - 데이터 누수 방지)
# =========================================================
...
print("\n===== Test 원본 분포 (절대 오염되지 않음) =====")
print(test_df["label"].value_counts())

# =========================================================
# 2. Oversampling (오직 Train 데이터만 증식)
# =========================================================

print("\n===== Train 데이터 Oversampling 시작 =====")

# Train 셋 규모에 맞게 Target Count 조정 (예시: 정상 600, 우울증 단계별 300)
target_counts = {
    0: 600,
    1: 300,
    2: 300,
...
for label in sorted(train_df["label"].unique()):
    ...
    if current_count > target_count:
        sampled_df = resample(class_df, replace=False, n_samples=target_count, random_state=SEED)
    elif current_count < target_count:
        sampled_df = resample(class_df, replace=True, n_samples=target_count, random_state=SEED)
    else:
        sampled_df = class_df
...
# 전역(Global) 텐서로 선언하여 Trainer에서 사용 가능하도록 함
...
class WeightedTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        # 수정: 계산해둔 가중치 텐서를 실제로 Loss 함수에 반영!
